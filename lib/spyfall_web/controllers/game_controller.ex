defmodule SpyfallWeb.GameController do
  ...
  def join_new(conn, %{"id" => game_id}) do
    with {:ok, game, player} <- session_game_and_player(conn),
  ...
  def join_new(conn, _params) do
    case session_game_and_player(conn) do
      {:ok, game, player} -> redirect(conn, to: Routes.game_path(conn, :room))
      _ -> render(conn, "join.html", changeset: Game.Form.join_changeset(%{}))
    end
  end
